		...
		<div class="row">
			...
			<div class="col-lg-6 col-5">
				<div class="right_area text-right">
					<a href="{{ route('admin.blogs') }}" class="btn btn-default">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
    	...
    	<div class="row">
    		<div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
    			<div class="card">
    				<h5 class="card-header">Update Blog Details Here.</h5>
    				<hr class="my-0" />
    				<div class="card-body">
    					<form action="{{ route('admin.blogs.edit', ['id' => $blog->id]) }}" method="post" class="form-validation">
    						<!--!! CSRF FIELD !!-->
    						{{ csrf_field() }}
    						<div class="row">
    							<div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
    								<div class="form-group">
								      		...
								      		@if($c->sub_categories()->count() > 0)
								      			<optgroup label="{{ $c->title }}">
								      				@foreach($c->sub_categories as $c)
								      					<option 
								      						value="{{ $c->id }}" 
								      						{{ $selectedCategories && in_array($c->id, $selectedCategories)  ? 'selected' : '' }}
								      					>{{ $c->title }}</option>
								      				@endforeach
								      			</optgroup>
								      		@else
    									...
    									<label id="category[]-error" class="error" for="category[]">@error('category[]') {{ $message }} @enderror</label>
    								</div>
    							</div>
    							<div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
    								<div class="form-group">
    									<label class="form-label">Title</label>
    									<input type="text" class="form-control" name="title" value="{{ old('title', $blog->title) }}" placeholder="Enter title" required />
    							...
    							<div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
    								<div class="form-group">
    									...
    									<textarea id="editor1" class="form-control" name="description" placeholder="Enter description">{{ old('description', $blog->description) }}</textarea>
    							...
    							<div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
    								<div class="form-group">
    									...
    									<textarea class="form-control" name="meta_description" placeholder="Enter meta description">{{ old('meta_description', $blog->meta_description) }}</textarea>
    									...
    									>
    										{{-- 
    											In case of single image use in upload-section class after  
												 {{ $blog->image ? 'd-none' : '' }} 
    										--}}
    										<div class="upload-section {{ $blog->image ? 'd-none' : '' }}">
    							...
    							<div class="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
    								<div class="form-group">
    									<label class="form-label">Publish or Unpublish Blog</label>
    									<div class="form-check form-switch mt-2">
    										<input type="hidden" name="status" value="0">
    			                    		<input type="checkbox" name="status" class="form-check-input" id="status" value="1" {{ old('status', $blog->status) == 1 ? 'checked' : '' }}/>
    										<label class="form-check-label" for="status">Do you want to publish this Blog ?</label>
    			                      	</div>
    			                    </div>
    			                </div>
    						</div>
